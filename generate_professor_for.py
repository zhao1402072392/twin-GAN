...
from sequence_generator_professor import SequenceGenerator
import os
...
def main(args):
    ...
    # Load model
    file_dir = "checkpoints/realmyzhenup10shrink07/"
    ...
    # g_model_path = 'checkpoints/professorwarmGDorilimDHumlimPG/sampling_3.960.epoch_9.pt'
    for filename in FileName:
        g_model_path = os.path.join(file_dir, filename)
        ...
        if use_cuda:
            translator.cuda()
        pretext = file_dir + os.path.splitext(filename)[0]+'.txt'
        # os.mkdir(pretext)
        with open(pretext, 'wb') as translation_writer:
