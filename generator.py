...
class LSTMDecoder(nn.Module):
    ...
    def forward(self, prev_output_tokens, encoder_out, incremental_state=None):
        ...
        # initialize previous states (or get from cache during incremental generation)
        cached_state = utils.get_incremental_state(self, incremental_state, 'cached_state')
        # print("cache1111111",cached_state.size())
        ...
        for j in range(seqlen):
            ...
            input = torch.cat((x[j, :, :], input_feed), dim=1)  # input = [len, 2*dim]
            # input_feed = [len,dim]

            for i, rnn in enumerate(self.layers):
                # recurrent cell
                hidden, cell = rnn(input, (prev_hiddens[i], prev_cells[i]))
                # print("111111111111111", i,hidden.size())

                # hidden state becomes the input to the next layer
                input = F.dropout(hidden, p=self.dropout_out, training=self.training)# input = [batch,dim]
            ...
            out = F.dropout(out, p=self.dropout_out, training=self.training)  # out = [batch,dim]
            # input feeding
            input_feed = out
        ...
        x = torch.cat(outs, dim=0).view(seqlen, bsz, embed_dim)  # x = [len, batch, dim]
        # T x B x C -> B x T x C
        x = x.transpose(1, 0)
        # srclen x tgtlen x bsz -> bsz x tgtlen x srclen
        attn_scores = attn_scores.transpose(0, 2)

        x = self.fc_out(x)  # x = [batch, len, num_vocab]
        return x, attn_scores
