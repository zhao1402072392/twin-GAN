...
class LSTMModel(nn.Module):
    ...
    def forward(self, phase, epoch, sample):
        ...
        decoder_out, attn_scores, p = self.decoder(phase, epoch, self.fixed_max_len, sample['net_input']['prev_output_tokens'], encoder_out)
...
class LSTMDecoder(nn.Module):
    def __init__(self, dictionary, encoder_embed_dim=512, embed_dim=512,
        ...
        # self.linear_p = Linear(encoder_embed_dim, 1)
        # self.belta = 1
        # self.gamma = 0.5

        # w_decoder_state = torch.zeros(encoder_embed_dim, 1).float()
        # bias = torch.zeros(1).float()
        # self.w_decoder_state = Parameter(w_decoder_state)
        # self.bias = Parameter(bias)

        # def calculate_p(self, epoch, pre_de_sate):
        #     batch_p = torch.mm(pre_de_sate, self.w_decoder_state)
        #     batch_p_ = batch_p + self.bias
        #     return p
        # alpha = nn.init.normal_(torch.zeros(1))
        # belta = nn.init.normal_(torch.zeros(1))

        # self.alpha = Parameter(torch.zeros(1), requires_grad=True)
        # self.belta = Parameter(torch.zeros(1), requires_grad=True)
        # self.alpha.grad.data.zero_()
        # self.belta.grad.data.zero_()
    ...
    def calculate_p(self, epoch, pre_label, pre_output):
        ...
        # batch_p = self.linear_p(pre_de_hidden)
        # batch_p_ = torch.sigmoid(batch_p)
        # print("batch_P//////", sum(batch_p) / len(batch_p_))
        # p = torch.add(self.belta * relativity, self.gamma * batch_p_)
        p_average = sum(relativity)/len(relativity)
        now = datetime.datetime.now()
        if now == 10:
            print("re", p_average)
        # final_p = torch.sigmoid(p_average)
        return p_average
