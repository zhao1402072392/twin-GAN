        ...
        for i, sample in enumerate(trainloader):
            ...
            if num_update%5 == 0:
                ...
                if random.random() > 0.5:
                    ...
                    fake_d_loss = d_criterion(fake_disc_out.squeeze(1), fake_labels)
                    fake_acc = torch.sum(torch.round(fake_disc_out).squeeze(1) == fake_labels).float() / len(
                        fake_labels)
                    d_loss = fake_d_loss
                    acc = fake_acc
                else:
                    true_disc_out = discriminator(src_sentence, true_sentence)
                    true_d_loss = d_criterion(true_disc_out.squeeze(1), true_labels)
                    true_acc = torch.sum(torch.round(true_disc_out).squeeze(1) == true_labels).float() / len(
                        true_labels)
                    d_loss = true_d_loss
                    acc = true_acc
            ...
            if num_update % 10000 == 0:
                ...
                torch.save(generator,
                           open(checkpoints_path + f"numupdate_{num_update/10000}k.joint_{g_logging_meters['valid_loss'].avg:.3f}.pt",
